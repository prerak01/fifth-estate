...
import { State } from "../managed/ballot/contract/index.cjs";

setNetworkId(NetworkId.Undeployed);

describe("Ballot smart contract", () => {
  it("generates initial ledger state deterministically", () => {
    const key = randomBytes(32);
    const simulator0 = new BallotSimulator(key);
    const simulator1 = new BallotSimulator(key);
    expect(simulator0.getLedger()).toEqual(simulator1.getLedger());
  });

  it("properly initializes ledger state and private state", () => {
    const key = randomBytes(32);
    const simulator = new BallotSimulator(key);
  ...
  it("lets you set a message", () => {
    const simulator = new BallotSimulator(randomBytes(32));
    const initialPrivateState = simulator.getPrivateState();
    const message =
